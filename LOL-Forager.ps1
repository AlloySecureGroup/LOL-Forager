...
'@
                ...
                $validationInterface='Discovered executable is launched; loaded DLL/modules are captured from that process and descendants. Supplemental PoC reproduces Assembly.LoadFrom + reflection separately.'
                $validationCommand='& "{0}"' -f $targetObserver
                $expectedResult='The exact discovered executable is started. TargetEvidence\LoadedModules-*.csv records DLL/modules actually observed in that process or its descendants. The separate Run-PluginLoadingPoC.ps1 only demonstrates the generic managed plugin-loading primitive and is not counted as proof that the target accepts arbitrary plugins.'
                $notes='DIRECT TARGET VALIDATION: Run-TargetBehaviorObservation.ps1 executes the discovered binary itself and records module loads, descendants, and TCP connections. Static LoadLibrary/GetProcAddress strings remain potential capability until corroborated by observed behavior or a documented product-specific plugin interface. SUPPLEMENTAL ONLY: Run-PluginLoadingPoC.ps1 demonstrates what dynamic managed loading looks like, but does not validate the target application.'
            }
...
'@
    ...
    @(
        ...
        '  2. A safe direct capability-specific test runs when one is known.',
        '  3. Assert-ForagerClaims.ps1 classifies each capability claim against observed evidence.',
        '  4. CapabilityAssertions.csv records StaticOnly, ObservedCorroboration,',
...
foreach($file in $candidates){
    ...
    $validation=[pscustomobject]@{Directory='';ValidationType='';ValidationInterface='';ValidationCommand='';DirectValidationCommand='';ObservationScript='';AssertionScript='';ValidationExpectedResult='';Readme=''}
