...
'@
        ...
        default {
            ...
            elseif($Capabilities -contains 'PluginLoading' -or $Capabilities -contains 'AssemblyLoading'){
                $validationType='PluginLoaderManual'
                $validationInterface='Dynamic DLL/assembly/plugin loading'
                $expectedResult='Application-specific plugin loading should load an approved benign module whose initialization path produces a Hello World marker.'
                $notes='The scanner detected a loader primitive but cannot prove which public entry point reaches it. Manual interface validation is required.'
            }
    ...
    @(
        ...
        '  - Generated tests are local and benign; no remote payload retrieval is used.',
